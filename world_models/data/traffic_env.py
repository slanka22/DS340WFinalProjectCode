...
import cv2 #Added for Grayscale
...
def generate_data(rollouts, data_dir, noise_type): # pylint: disable=R0914
    ...
    env = MetaSUMOEnv(dict(begin_time=0,
                            delta_time=5,
                            window_size=(64, 64), #Modified for DreamerV2
                            stack_size=1,
                            sumo_gui=False,
    ...
    for i in range(start_rollouts,rollouts):
        ...
        while True:
            ...
            grayscaleImage = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY) #https://cloudinary.com/guides/image-effects/a-guide-to-converting-images-to-grayscale-with-python-introduction

            s_rollout.append(grayscaleImage)
            r_rollout.append(r)
